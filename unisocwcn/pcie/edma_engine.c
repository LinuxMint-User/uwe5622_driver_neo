...
int edma_init(struct wcn_pcie_info *pcie_info)
{
	...
	do {
		...
		#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 18, 0)
		{
			...
			ret = syscall(__NR_sched_setattr, (pid_t)0, &attr, 0);
			PCIE_INFO("sched_setattr(SCHED_FIFO), prio:90, ret:%d\n", ret);
		}
		#else
		{
			struct sched_param param;
			param.sched_priority = 90;
			ret = sched_setscheduler(edma->isr_func.entity, SCHED_FIFO, &param);
