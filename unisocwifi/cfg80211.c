...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 18, 0)
	priv = timer_container_of(priv, t, scan_timer);
#else
	priv = from_timer(priv, t, scan_timer);
#endif
#else
...
static int sprdwl_cfg80211_set_wiphy_params(struct wiphy *wiphy, int params, u32 changed)
{
	struct sprdwl_priv *priv = wiphy_priv(wiphy);
	u32 rts = 0, frag = 0;
